...
- [ ] 8.5 在 `services/session_manager.py` 或 `routers/sessions.py` / `ws.py` 创建 session 时检查 RAG config，按 `enabled` 注入三工具

## 9. Gateway REST API

- [ ] 9.1 `POST /api/rag/rebuild`：触发全量重建（异步），返回 job_id
- [ ] 9.2 `POST /api/rag/update`：触发增量（异步），返回 job_id
- [ ] 9.3 `POST /api/rag/cancel`：取消进行中的 job
- [ ] 9.4 `GET /api/rag/status`：返回 stats + watcher 状态 + 最近活动 + 今日费用
- [ ] 9.5 `POST /api/rag/search`：playground 调用，参数 `{query, top_k?}`
...
- [ ] 9.13 `GET /api/rag/stream`：统一 SSE 进度通道
- [ ] 9.14 `POST /api/rag/purge`：清空索引（删 DB 文件 + 内存状态）

## 10. Web UI: /rag 页面

- [ ] 10.1 添加路由 `/rag` 到 `HLAgent/web/src/App.tsx`（或路由配置）
- [ ] 10.2 SSE 客户端 hook `useRagStream()`：订阅 `/api/rag/stream`，按 stage/source 分发到全局状态
- [ ] 10.3 状态卡片组件：显示 chunks/files/size/last_indexed/today_cost
- [ ] 10.4 操作按钮组件：[手动重建] [手动增量] [取消]；watcher 切换 toggle；预算配置（日额度 + 超额行为）
- [ ] 10.5 进度条组件：根据 SSE 事件实时更新；显示当前 stage/file/速度
...
- [ ] 10.15 Playground 区块：输入框 + [搜索] + 结果列表（可展开 content / 跳转源文件）
