...
- `POST /api/sessions` — 创建新会话，支持传递 `model`、`cwd`、`permission_mode`、`system_prompt`、`max_turns` 参数
- `GET /api/sessions/{session_id}` — 获取会话当前完整 AppState（含 model、provider、auth_status、cwd、fast_mode、plan_mode 等）
- `DELETE /api/sessions/{session_id}` — 关闭会话并清理资源
...
#### Scenario: 删除会话
- **WHEN** 客户端发送 `DELETE /api/sessions/{session_id}`
- **THEN** 返回 HTTP 204，Gateway 清理 Agent 运行时，释放资源
