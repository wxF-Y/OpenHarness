...
class SwarmSendMessageInput(BaseModel):
    """Arguments for sending a message to a swarm member."""
...
class SwarmSendMessageTool(BaseTool):
    """Send a follow-up message or instruction to a running swarm team member.
    ...
    description = (
        "Send a message or follow-up instruction to a swarm team member. "
    ...
    input_model = SwarmSendMessageInput

    async def execute(self, arguments: SwarmSendMessageInput, context: ToolExecutionContext) -> ToolResult:
        del context
